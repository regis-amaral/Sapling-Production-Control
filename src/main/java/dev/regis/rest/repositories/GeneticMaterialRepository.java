...
public interface GeneticMaterialRepository extends JpaRepository<GeneticMaterial, Long>  {
    ...
    // Page<GeneticMaterial> findByNameContaining(String partName, Pageable pageable);

    @Query("FROM GeneticMaterial gm WHERE LOWER(gm.name) LIKE %:partName%") // Correção aqui
